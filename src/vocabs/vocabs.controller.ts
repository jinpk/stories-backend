...
export class VocabsController {
  ...
  @Get('random/:level/by_level')
  @ApiOperation({
    summary: '레벨별 전체 vocab 조회',
    description: 'max - 요청 최대 랜덤 단어 수 default 1',
  })
  @ApiOkResponse({ type: [VocabDto] })
  async listVocabsByLevel(
    ...
    return await this.vocabsService.getRandomVocabsByLevel(level, max);
  }
  ...
  @ApiOperation({
    ...
    description:
      "contentsSerialNumber 사용, 각 스토리에 해당하는 단어 목록 호출, 'Y'인 경우 핵심단어만, 'N'인 경우 전체 단어",
  })
  ...
  async saveUserReviewVocab(
    ...
    return await this.vocabsService.createReviewVocab(
      req.user.id,
      vocabId,
      level,
    );
  ...
  async updateUserReviewVocab(
    ...
    if (
      !(await this.vocabsService.existReviewVocabById(
        req.user.id,
        reviewvocabId,
      ))
    ) {
    ...
    return await this.vocabsService.updateReviewVocabById(
      req.user.id,
      reviewvocabId,
    );
  }
